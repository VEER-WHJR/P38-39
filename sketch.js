...
function setup() {
  ...
  kangaroo = createSprite(width/16,height/2,20,50);
  kangaroo.addAnimation("running", kangaroo_running);
  kangaroo.addAnimation("collided", kangaroo_collided);
  kangaroo.scale = 0.15;
  kangaroo.setCollider("circle",0,0,300)

  invisibleGround = createSprite(width/2,height - 50,1600,10);
...
function draw() {
  ...
  if (gameState===PLAY){
   ...
   console.log(kangaroo.y)
    if(keyDown("space")&& kangaroo.collide(invisibleGround)) {
      jumpSound.play();
...
if(gameState === END && mousePressedOver(restart)){
  ...
  drawSprites();
  text("Score : "+score,width-60,10);
  if(gameState===PLAY && score === 1){
    kangaroo.velocityY = 0;
    jungle.velocityX = 0;
    obstaclesGroup.setVelocityXEach(0);
    shrubsGroup.setVelocityXEach(0);
    ...
    obstaclesGroup.setLifetimeEach(-1);
    shrubsGroup.setLifetimeEach(-1);
    text("Congratulations!! You win the game !!",width/2,height/2);
  }
...
function spawnShrubs() {
  ...
  if (frameCount % 150 === 0) {
    ...
    // var shrub = createSprite(camera.position+500,330,40,10);
     var shrub = createSprite(camera.position.x+(width/2+100),height-70,40,10);
    // var shrub = createSprite(camera.positionX+500,330,40,10);
    // var shrub = createSprite(Camera.position.x+500,330,40,10);

    shrub.velocityX = -(6 + 3*score/30)
    shrub.scale = 0.6;
    ...
    shrub.scale = 0.05;
    shrub.lifetime = (width/shrub.velocityX);
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    ...
    // var obstacle = createSprite(Camera.Position.x+400,330,40,40);
     var obstacle = createSprite(camera.position.x+(width/2),height-70,40,40);
    // var obstacle = createSprite(camera.position.x.400,330,40,40);
